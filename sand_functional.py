...
def is_move_ok(grid, x_from, y_from, x_to, y_to):
    ...
    True
    """
    grid_border_x = grid.width
    grid_border_y = grid.height
    
    if DEBUG == True: print("DEBUG: Running 'is_move_ok'...")
    
    if 0 <= x_to < grid_border_x and 0 <= y_to < grid_border_y: pass
    else: 
        if DEBUG == True: print("DEBUG: Out of bounds.")
        return
    
    if grid.get(x_to, y_to) == EMPTY: pass
    else: 
        if DEBUG == True: print("DEBUG: Destination is not empty.")
        return
    
    down = (x_from, y_from + 1)
    down_right = (x_from + 1, y_from + 1)
    down_left = (x_from - 1, y_from + 1)
    destination = (x_to, y_to)
    
    if destination == down: 
        if DEBUG == True: print("DEBUG: Can move down.")
        return True
    
    if destination == down_right and (grid.get(x_to, y_to) == EMPTY) and (grid.get(x_to, y_to - 1) == EMPTY):
        if DEBUG == True: print("DEBUG: Can move down-right.")
        return True
    
    if destination == down_left and (grid.get(x_to, y_to) == EMPTY) and (grid.get(x_to, y_to - 1) == EMPTY):
        if DEBUG == True: print("DEBUG: Can move down-right.")
        return True
    
    if DEBUG == True: print("DEBUG: Unable to move.")
    return False
...
def do_move(grid, x_from, y_from, x_to, y_to):
    ...
    """
    grid.set(x_to, y_to, grid.get(x_from, y_from))
...
def do_gravity(grid, x, y):
    ...
    :param grid: a grid [theoretically] with rocks and sand
    ...
    """
    defined_pixel_value = grid.get(x, y)
    
    if defined_pixel_value != 's': return grid
